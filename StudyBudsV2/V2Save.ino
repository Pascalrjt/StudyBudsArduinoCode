...
LeafState leafStates[3] = {LEAF_OPEN, LEAF_OPEN, LEAF_OPEN};
...
bool closingDelayArmed[3] = {false, false, false};
uint32_t closingReadyAtMs[3] = {0, 0, 0};
...
void displayStudyMode() {
  ...
  // Bottom row: "X/3 leaves used"
  lcd.setCursor(0, 1);
  lcd.print(closedCount);
  lcd.print("/3 leaves used");
...
void setup() {
  ...
  for (int i = 0; i < 3; ++i) pinMode(LEAVES[i].buttonPin, INPUT_PULLUP);
...
void loop() {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    if (i < 2) delay(INTER_PING_DELAY_MS);
  }
      ...
      if (leafStates[i] == LEAF_OPEN && distances[i] <= TRIP_CM) {
        ...
        if (!closingDelayArmed[i]) {
          ...
          if (!sessionActive) {
            sessionActive = true;
            sessionStartMs = now;
            for (int j = 0; j < 3; ++j) votePressed[j] = false; // clear votes for new session
          }
        }
